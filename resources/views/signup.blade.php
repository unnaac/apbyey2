...
<body class="bg-gradient-to-br from-neutral-50 via-white to-telusafe-50 min-h-screen flex items-center justify-center p-4 sm:p-6">
    ...
    <div class="w-full max-w-md relative z-10 animate-scale-in">
        ...
        <div class="glass-effect rounded-4xl shadow-strong p-8 border-2 border-white/50 hover-lift">
            ...
            <div id="card-2" class="stagger-animation space-y-6 card-hidden">
                ...
                <!-- Password -->
                <div class="space-y-2 password-field-group">
                    ...
                    <div class="relative" id="hilangin-password">
                        <input 
                            type="password" 
                            id="password" 
                            name="password" 
    ...
    <script>
        ...
        window.addEventListener('DOMContentLoaded', () => {
            ...
            if (userData) {
                ...
                document.getElementById('hilangin-password').style.display = 'none'; 
            }
    ...
    if (userData) {
        console.log("Pengguna Google terdeteksi. Menyembunyikan field password.");

        // Cari semua elemen dengan class 'password-field-group'
        const passwordFields = document.querySelectorAll('.password-field-group');
        
        // Loop melalui setiap elemen dan sembunyikan
        passwordFields.forEach(field => {
            field.style.display = 'none';
        });

        // Hapus juga atribut 'required' dari input password agar form valid
    ...
    // Cek apakah ada data dari alur login Google di sessionStorage
    const googleUserDataJSON = sessionStorage.getItem('googleUser');
    ...
    try {
        ...
        // Ambil nilai dari form yang dibutuhkan di kedua alur
        const username = document.getElementById('usernameSSO').value.trim();
        const nik = document.getElementById('nik').value.trim();
        const phone = document.getElementById('phone').value.trim();
        const status = document.getElementById('status').value;

        if (googleUserDataJSON) {
            // --- ALUR 1: Melengkapi profil untuk pengguna Google ---
            console.log("Completing profile for Google user...");
            const googleUser = JSON.parse(googleUserDataJSON);
            ...
            // --- ALUR 2: Pendaftaran biasa dengan email dan password ---
            console.log("Performing standard email/password signup...");
        ...
        // Kumpulkan semua data untuk disimpan ke database
        ...
        await database.ref('users/' + uid).set(userDataToSave);

        // Hapus data Google dari session jika ada, setelah berhasil disimpan
        if (googleUserDataJSON) {
        ...
        setTimeout(() => {
            ...
            window.location.href = '/login'; // atau '/dashboard' jika ingin langsung masuk
        }, 2000);

    } catch (error) {
        console.error("Error during sign up:", error);
        let errorMessage = error.message; // Pesan default

        // Pesan error yang lebih spesifik dari Firebase
        if (error.code) {
            switch (error.code) {
